...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      bool isItemChecked(WidgetTester tester, String itemLabel) {
      ...
      expect(isItemChecked(tester, 'All'), isFalse);
      ...
      expect(isItemChecked(tester, 'All'), isTrue);

      // Deselecting a single item should uncheck 'All' again.
      await tester.tap(find.text('Item2').last);
      await tester.pumpAndSettle();
      expect(isItemChecked(tester, 'All'), isFalse);
      expect(isItemChecked(tester, 'Item1'), isTrue);
      expect(isItemChecked(tester, 'Item3'), isTrue);
      expect(isItemChecked(tester, 'Item4'), isTrue);
      ...
      for (final item in ['All', 'Item1', 'Item2', 'Item3', 'Item4']) {
        expect(isItemChecked(tester, item), isFalse);
      }
    },
  );
}
